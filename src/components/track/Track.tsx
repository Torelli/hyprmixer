...
export default function Track() {
	const { track, isDiscMode, setDiscMode, currentPlayer, isPlayerListOpen } = useContext(PlayerContext)
	...
	return <div className={`${isPlayerListOpen && 'opacity-0'} pt-12 sm:pt-8 md:p-0 text-white flex flex-col items-center justify-center gap-4 transition-all`}>
		...
		<div className="flex flex-col gap-2 w-64">
			...
			<div className="min-h-12 sm:min-h-20 text-sm sm:text-lg md:text-2xl flex items-center justify-around">
				...
				<button onClick={() => togglePlayPause(currentPlayer.name)} className={`hover:bg-gray-900/60 ${isPlaying ? "py-2 px-4" : "py-3 pl-5 pr-4"} rounded-full focus-visible:outline-white transition-all`}>
					{isPlaying ? <FontAwesomeIcon icon={faPause} /> : <FontAwesomeIcon icon={faPlay} />}
				</button>
